		...
		<script type="text/javascript">
			...
			function getData(config) {
				try {
					...
					logger.debug("LineView Widget: getData(): url: " + url);
					...
					if (result.length == 0) {
						...
						for (i = 0; i < result.length; i++) {
							myData.push({
								name:result[i][0], 
								untasked: result[i][1], 
								unfulfilled:result[i][2], 
								accomplished:result[i][3]
							});
						}
					}
					
					logger.debug("LineView Widget: getData(): myData: " + myData);
				} catch(err) {
					logger.debug("LineView Widget: getData(): err: " + err);
				}
			...
			function drawGridObject(configString) {
				try {
					...
					logger.debug("LineView Widget: drawGridObject(): err: " + err);
				} 
			}
		
			var update = function(sender, msg) {
				try {
					logger.debug('LineView Widget: update(): msg: ' + msg);
					store.loadData(getData(msg));
				} catch(err) {
					logger.debug('LineView Widget: update(): err: ' + err);
				}
			}
